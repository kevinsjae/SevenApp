...
+(void)keepTaskInBackgroundForPhotoUpload     {
    ...
    self.sharedBackgroundHelper.backgroundTask = [[UIApplication sharedApplication] beginBackgroundTaskWithExpirationHandler:^{
        DebugLog(@"Background handler called. Not running background tasks anymore.");
...
+(void)calculateNextNumber
{
    ...
    {
        DebugLog(@"Result: %@", [NSString stringWithFormat:@"Position %lu = %@", (unsigned long)self.sharedBackgroundHelper.position, self.sharedBackgroundHelper.current]);
    }
    else
    {
        DebugLog(@"App is backgrounded. Next number = %@", [NSString stringWithFormat:@"Position %lu = %@", (unsigned long)self.sharedBackgroundHelper.position, self.sharedBackgroundHelper.current]);
        DebugLog(@"Background time remaining = %.1f seconds", [UIApplication sharedApplication].backgroundTimeRemaining);
    }
}
